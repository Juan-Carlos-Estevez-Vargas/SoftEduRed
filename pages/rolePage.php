...
class RolePage extends BasePage
{
    ...
    /**
     * Renders the new role HTML based on the current request parameters.
     *
     * @throws Some_Exception_Class If the request action is not set to 'ver' or 'edit'.
     */
    private function renderNewRole()
    {
        ...
        if (!empty($_POST['role'])) {
            $this->roleService->register($_POST['role']);
        }
    ...
    /**
     * Renders the update role form if the 'action' query parameter is not empty and the 'id' variable is not empty.
     *
     * @throws Some_Exception_Class description of exception
     */
    private function renderUpdateRole(int $id)
    {
        if (!empty($_GET['action']) && !empty($id)) {
            $role = $this->roleService->getRoleById($id);

            if (!$role) {
                // Role not found, handle the error here
                ...
                <div class="row">
                    <div class="col-md-12">
                        <form action="?action=update" method="post" enctype="multipart/form-data">
                            ...
                            <div>
                                <input type="text" name="id_course" value="<?php echo $role['id_role']?>" style="display: none;">
                            </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-outline">
                                        <input type="text" name="role" class="form-control" style="text-transform:uppercase;"
                                            value="<?php echo $role['description']?>" maxlength="15" disabled required />
                                        <label class="form-label" for="role">Tipo de Rol:</label>
                                    </div>
                                </div>
     ...
     *
     * This function retrieves the role records from the database and displays them in a table.
     ...
     */
    private function renderRoleList()
    {
      if (!isset($_REQUEST['action']) || ($_REQUEST['action'] !== 'ver' && $_REQUEST['action'] !== 'edit'))
      {
        $data = $this->roleService->getRoleListData(isset($_GET['page']) ? $_GET['page'] : 1);
     ...
     *
     * This function calls three other functions to render the new role,
     * update role, and role list.
     *
     ...
     */
    private function handleUpdateRole()
    {
        if (isset($_POST['id_role'], $_POST['role'], $_POST['state'])) {
            $this->roleService->update($_POST['id_role'], $_POST['role'], $_POST['state']);
        }
    }
     ...
     */
    private function handleDeleteRole()
    {
        if (isset($_GET['id_role'])) {
            $this->roleService->delete($_GET['id_role']);
        }
    }
}

$pageTitle = "Rol";
$id = isset($_GET['id_role']) ? $_GET['id_role'] : null;
$rolePage = new RolePage($pageTitle, $id);
$rolePage->render();
?>